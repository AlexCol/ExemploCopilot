# Gabarito dos Exercícios - Data Types

## ✅ Soluções Comentadas

---

## Exercício 1: SERIAL vs UUID
...
- UUID: IDs únicos globalmente (ex: a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11)
- SERIAL é menor (4 bytes) vs UUID (16 bytes)
- UUID é ideal para sistemas distribuídos
*/
```
...
-- Forçar override (se realmente necessário)
...
CONCLUSÃO:
- ALWAYS: Mais restritivo, força uso da sequence
- BY DEFAULT: Flexível, permite override manual
*/
```
...
('Conferência Nova York', '2025-11-18 10:00:00-05'),
('Lançamento Tóquio', '2025-11-18 23:00:00+09'),
('Reunião Londres', '2025-11-18 15:00:00+00');

-- Consultar convertendo para horário de Brasília
SELECT 
    descricao,
...
- A conversão para timezone é feita na apresentação
- Todos os eventos são comparáveis independente do timezone de origem
*/
...
```sql
-- Usando a tabela do exercício anterior

-- a) Dias desde cada evento até hoje
SELECT 
    descricao,
    timestamp_evento,
...
-- d) Diferença em horas entre primeiro e último evento
SELECT 
...
SELECT 
    ...
    CASE 
        WHEN (prazo - tempo_estimado) <= NOW() THEN '⚠️  ATRASADO'
        WHEN (prazo - tempo_estimado) <= NOW() + INTERVAL '1 day' THEN '🔔 URGENTE'
        ELSE '✅ OK'
    END AS status
FROM tarefas
ORDER BY deve_iniciar_em;

-- c) Tarefas que precisam começar hoje ou já deveriam ter começado
SELECT 
    titulo,
    prazo - tempo_estimado AS deve_iniciar_em
FROM tarefas
...
('Escritório Principal', '192.168.1.0/24');

-- c) Criar função de verificação
...
SELECT * FROM verificar_acesso('192.168.1.50'::INET);    -- TRUE (escritório)
SELECT * FROM verificar_acesso('200.1.1.1'::INET);       -- FALSE (não autorizado)
SELECT * FROM verificar_acesso('172.16.10.25'::INET);    -- TRUE (VPN)

-- Verificação alternativa sem função
SELECT 
...
```sql
-- Setup (já fornecido no exercício)
...
-- a) Dispositivos cuja rede contém IP 192.168.1.150
SELECT nome, rede
FROM dispositivos
...
-- c) Endereço de broadcast de cada rede
SELECT 
    nome,
    rede,
...
EXPLICAÇÃO:
- && retorna TRUE se as redes têm interseção
- 192.168.1.0/24 e 192.168.2.0/24 são redes diferentes
- 192.168.1.0/25 está contida em 192.168.1.0/24
*/
```
...
/*
CONCLUSÕES:
- MONEY é mais compacto mas depende de locale
- NUMERIC é mais portável e preciso
- Para produção, prefira NUMERIC para valores monetários
*/
```
...
('Corrigir bug crítico', FALSE, TRUE, TRUE, FALSE),
('Refatorar código', FALSE, FALSE, NULL, FALSE),
('Atualizar docs', TRUE, FALSE, FALSE, FALSE),
('Code review', TRUE, FALSE, TRUE, FALSE),
('Deploy produção', FALSE, TRUE, FALSE, FALSE),
('Testes unitários', TRUE, TRUE, TRUE, FALSE),
('Reunião cliente', TRUE, FALSE, TRUE, TRUE),
('Backup database', FALSE, FALSE, TRUE, FALSE),
('Análise performance', FALSE, FALSE, NULL, FALSE);

-- b) Tarefas pendentes
SELECT * FROM tarefas_projeto
WHERE NOT concluida AND NOT arquivada;

-- c) Tarefas urgentes aguardando aprovação
SELECT * FROM tarefas_projeto
WHERE urgente AND aprovada IS NULL;

-- d) Tarefas concluídas mas não aprovadas
SELECT * FROM tarefas_projeto
WHERE concluida AND aprovada = FALSE;

-- e) View de classificação
...
SELECT 
    ...
    CASE
        ...
        WHEN concluida AND aprovada THEN '✅ Concluída e Aprovada'
        WHEN concluida AND NOT COALESCE(aprovada, TRUE) THEN '⚠️  Concluída mas Reprovada'
        WHEN concluida AND aprovada IS NULL THEN '🔄 Aguardando Aprovação'
        WHEN urgente AND NOT concluida THEN '🚨 Urgente'
        ELSE '📋 Pendente'
    END AS status
...
-- (em produção, seriam conexões diferentes)
...
-- Verificar: todos os IDs são únicos!
SELECT id, servidor_origem, valor FROM pedidos;

-- Comparação com SERIAL (problema)
...
-- Se 3 servidores gerarem IDs localmente, haverá conflito ao mesclar!
-- Com UUID, não há conflito mesmo em sistemas totalmente distribuídos
```
